...
const KEY_SERVICE_URL =
  process.env.KEY_SERVICE_URL || 'https://key.bnpanel.top/connect';
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: 'goku-backend' });
});
...
app.get('/api/history', async (req, res) => {
  ...
  if (!target || !allowUrl(target)) {
    return res.status(400).json({ error: 'Invalid history URL' });
  }
...
app.listen(PORT, () => {
  console.log(`GOKU backend listening on :${PORT}`);
});
